...
const saleTypePillsContainer = document.getElementById(
  "sale-type-pills-container"
);
...
const eventCategoriesContainer = document.getElementById(
  "event-categories-container"
);
const eventSaleTypePillsContainer = document.getElementById(
  "event-sale-type-pills"
);
const eventPhotoInput = document.getElementById("event-photo");
const eventTitleInput = document.getElementById("event-title");
const eventDescriptionInput = document.getElementById("event-description");
let eventAddressInput = document.getElementById("event-address-automcomplete");
    ...
    const buildInfoWindowContent = () => {
      ...
      const titles = cluster.markers
        ...
        .map((m) => {
          ...
          const escapedTitle = event
            ? event.title.replace(/</g, "&lt;").replace(/>/g, "&gt;")
            : "";
      ...
      return `
                    <div class="cluster-info-window">
                        ...
                        ${
                          moreCount > 0
                            ? `<p class="cluster-info-more">...and ${moreCount} more.</p>`
                            : ""
                        }
                    </div>`;
    };
...
const updateFavoriteUI = (eventId, isNowFavorite) => {
  ...
  const cardInList = eventsContainer.querySelector(
  ...
  if (
    ...
    currentlySelectedEventForDebug?.public_id === eventId
  ) {
...
const toggleFavorite = (event) => {
  ...
  if (
    document
      .querySelector('.sale-type-pill[data-sale-type="favorites"]')
...
debugOverlay.innerHTML =
  '<div id="debug-header">Debug</div><div id="debug-content"></div>';
...
const validateForm = () => {
  ...
  // First, check if the input field has any text.
  if (!isRequired(eventAddressInput)) {
    showError(eventAddressInput, "Address is required.");
    isFormValid = false;
  } else if (!submissionCoordinates) {
    // Then, check if a valid, geocoded address has been selected.
    showError(eventAddressInput, "Please select a valid address from the dropdown.");
    isFormValid = false;
  }
  ...
  if (!isRequired(eventEndInput)) {
    ...
    new Date(eventEndInput.value) <= new Date(eventStartInput.value)
  ) {
  ...
  if (
    eventCategoriesContainer.querySelectorAll('input[type="checkbox"]:checked')
      .length === 0
  ) {
...
eventPhotoInput.addEventListener("change", async () => {
  ...
  if (files.length > remainingSlots) {
    alert(
      `You can only add ${remainingSlots} more photo(s). Only the first ${remainingSlots} will be added.`
    );
  ...
  try {
    ...
    alert(
      "There was an error processing one or more images. Please try again with different files."
    );
  } finally {
...
submissionForm.addEventListener("submit", async (e) => {
  ...
  const payload = {
    ...
    item_categories: formValues
      .getAll("item_categories")
      .map((id) => parseInt(id, 10)),
    latitude: submissionCoordinates ? submissionCoordinates.lat : null,
    longitude: submissionCoordinates ? submissionCoordinates.lng : null,
  };
  ...
  try {
    ...
    submissionCoordinates = null; // Clear coordinates
...
function initializePanelResizer() {
  ...
  const onMouseUp = () => {
    ...
    ).getPropertyValue("--list-panel-width");
...
window.addEventListener("resize", () => {
  ...
  resizeTimeout = setTimeout(() => {
    if (isDesktop() && !document.getElementById("panel-resizer"))
      initializePanelResizer();
  }, 250);
});
...
const onPanelTouchStart = (e) => {
  ...
  if (
    ...
    e.target.closest(".header-action-btn, #collapse-button")
  ) {
    return;
  }
...
const onPanelTouchEnd = () => {
  ...
  if (!wasPanelDragged) {
    ...
    const transformMatrix = new WebKitCSSMatrix(
      window.getComputedStyle(listPanel).transform
    );
                  ...
                  .map(
                    ...
                    <div class="comment ${comment.user_id === currentUserId ? 'user-owned-comment' : ''}">
                        <p class="comment-text">${comment.comment_text
                          .replace(/</g, "&lt;")
                          .replace(/>/g, "&gt;")}</p>
                        <p class="comment-meta">Posted on ${new Date(
                          comment.created_at
                        ).toLocaleString()}</p>
                    </div>
                `
  ...
  if (directionsBtn) {
    directionsBtn.addEventListener("click", () => {
      ...
      const directionsUrl = `https://www.google.com/maps/dir/?api=1&origin=${
        userPos.lat
      },${userPos.lng}&destination=${encodeURIComponent(event.address)}`;
      window.open(directionsUrl, "_blank");
    });
  }

  document
    .getElementById("report-event-btn")
    .addEventListener("click", () => openReportModal(event));
  document
    .getElementById("add-to-calendar-btn")
    .addEventListener("click", () => handleAddToCalendar(event));
  document
...
function shareEvent(event) {
  ...
  const shareData = {
    ...
    text: `Check out this garage sale: ${event.description.substring(
      0,
      100
    )}...`,
    url: eventUrl,
...
function loadAndApplyFilters() {
  ...
  try {
    ...
    if (savedFilters.saleType) {
      document
        .querySelectorAll(".sale-type-pill")
        .forEach((p) => p.classList.remove("active"));
      const pillToActivate = document.querySelector(
        `.sale-type-pill[data-sale-type="${savedFilters.saleType}"]`
      );
      pillToActivate
        ? pillToActivate.classList.add("active")
        : document
            .querySelector('.sale-type-pill[data-sale-type="all"]')
    ...
    if (savedFilters.category) {
      document
        ...
        .forEach((p) => p.classList.remove("active"));
      const pillToActivate = document.querySelector(
        `.category-pill[data-category="${savedFilters.category}"]`
      );
      pillToActivate
        ? pillToActivate.classList.add("active")
        : document
            .querySelector('.category-pill[data-category="all"]')
            .classList.add("active");
    }
  } catch (e) {
...
function clearAllFilters() {
  ...
  document
    .querySelectorAll(".sale-type-pill")
    .forEach((p) => p.classList.remove("active"));
  const allSaleTypePill = document.querySelector(
    '.sale-type-pill[data-sale-type="all"]'
  );
  if (allSaleTypePill) {
    allSaleTypePill.classList.add("active");
    allSaleTypePill.scrollIntoView({
      behavior: "smooth",
      inline: "center",
      block: "nearest",
    });
  ...
  document
    .querySelectorAll(".category-pill")
    .forEach((p) => p.classList.remove("active"));
  const allCategoryPill = document.querySelector(
    '.category-pill[data-category="all"]'
  );
  if (allCategoryPill) {
    allCategoryPill.classList.add("active");
    allCategoryPill.scrollIntoView({
      behavior: "smooth",
      inline: "center",
      block: "nearest",
    });
...
function scrollToEventCard(eventId) {
  const cardToScrollTo = eventsContainer.querySelector(
    `.card[data-event-id='${eventId}']`
  );
  if (cardToScrollTo) {
...
function getFilteredEvents() {
  const selectedSaleType =
    document.querySelector(".sale-type-pill.active")?.dataset.saleType || "all";
...
function updateClearFiltersButton() {
  ...
  if (count > 0) {
    clearAllFiltersBtn.textContent = `Clear ${count} Filter${
      count > 1 ? "s" : ""
    }`;
...
function updateDebugOverlay() {
  ...
  let selectedEventHtml =
    '<strong>Selected Event:</strong><p style="margin:0;">None</p>';
  ...
  contentEl.innerHTML = `
                    ...
                    <strong>Clustering:</strong> ${
                      clusteringToggle.checked ? "On" : "Off"
                    }<br>
...
function filterAndDisplayEvents() {
  ...
  if (!bounds.isEmpty()) {
    ...
    if (ne.equals(sw)) {
      ...
      const defaultZoom = parseInt(
        localStorage.getItem(ZOOM_LEVEL_KEY) || "14",
        10
      );
      map.setCenter(ne);
...
async function initializeMap() {
  ...
  const defaultZoom = parseInt(
    localStorage.getItem(ZOOM_LEVEL_KEY) || "12",
    10
  );
  ...
  const radioToCheck = document.querySelector(
    `input[name="map-type"][value="${savedMapType}"]`
  );
  if (radioToCheck) {
...
function initAutocomplete() {
    ...
    autocomplete.addListener("place_changed", () => {
        ...
        if (!place.geometry) {
            submissionCoordinates = null; // Clear coordinates if address is invalid
            eventAddressInput.placeholder = 'Enter a valid address';
        } else {
            eventAddressInput.value = place.formatted_address;
            const location = place.geometry.location;
            submissionCoordinates = { lat: location.lat(), lng: location.lng() };
            validateForm(); // Re-validate to clear the error message
        }
    });
}
...
function createEventCard(event) {
  ...
  const favoriteButtonHtml = `
                    <button class="favorite-btn ${
                      isFavorite(event.public_id) ? "active" : ""
